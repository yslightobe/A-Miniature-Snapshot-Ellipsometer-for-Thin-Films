...
% created by：YSL
% date：2023.6
function  [N, C, S, M_basis, Igenerate] = compressed_sensing_diffConfig(config, Y, cos_delta_1,sin_delta1_s2,sin_delta1_c2, L, DCT, beta, sample_points)
% 用于通道型Stokes偏振仪的计算
% config: 仪器的实际配置
...
elseif config == "0-135-0-45"
    M_tri_N = diag(cos_delta_1);
    M_tri_C = -diag(sin_delta1_s2);
    M_tri_S = -diag(sin_delta1_c2);
elseif config == "0-45-90-45"
    M_tri_N = diag(cos_delta_1);
    M_tri_C = -diag(sin_delta1_s2);
    M_tri_S = diag(sin_delta1_c2);
elseif config == "0-135-90-45"
    M_tri_N = diag(cos_delta_1);
    M_tri_C = diag(sin_delta1_s2);
    M_tri_S = -diag(sin_delta1_c2);
elseif config == "0-45-0-135"
    M_tri_N = diag(cos_delta_1);
    M_tri_C = -diag(sin_delta1_s2);
    M_tri_S = -diag(sin_delta1_c2);
elseif config == "0-135-0-135"
    M_tri_N = diag(cos_delta_1);
    M_tri_C = diag(sin_delta1_s2);
    M_tri_S = diag(sin_delta1_c2);
elseif config == "0-45-90-135"
    M_tri_N = diag(cos_delta_1);
    M_tri_C = diag(sin_delta1_s2);
    M_tri_S = -diag(sin_delta1_c2);
elseif config == "0-135-90-135"
    M_tri_N = diag(cos_delta_1);
    M_tri_C = -diag(sin_delta1_s2);
    M_tri_S = diag(sin_delta1_c2);
else
...
cvx_begin
    ...
    Igenerate = (M_final*[x_optim_0;zero_set;x_optim_1;zero_set;x_optim_2;zero_set]+1);
