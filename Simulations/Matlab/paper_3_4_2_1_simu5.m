%% 3.4.2.1 仿真实验#5
% 方位角误差为1.5°、3.5°、1.3°、3.7°，温度从24℃波动到35℃，二氧化硅薄膜厚度为1.0um
clc
clear
close all
addpath(genpath(pwd))
...
hyper_paras2 = [0.05, 0.02, 0.01, 1, 1]; % 最后两个参数控制是否进行方位角和温度补偿
...
%% 读取测量时的延迟量（≠24℃），单位：度
phi2_realtime = spline(raw_wvl,dataTable.t35,wvl);
...
%% 生成样品参数和仿真测量光谱
[M_s, N, C, S] = rcwa_film(wvl/1000,1.0,65); % 微米(行向量)，微米，角度
